...
public class DataUtils {
    ...
    //-TomWang-----------------------------
    //byte转hex字符转int
    public static int Byte2int1(Byte inByte) { return (int)HexToInt(Byte2Hex(inByte));}


    //-TomWang-----------------------------
    //byte转hex字符转int
    public static int Byte2int(Byte inByte) { return (int)(inByte & 0xff);
    }
     ...
     * @return 结果
     */
    public static int returnByteLength(byte[] data) {
        int i = 0;
        for (; i < data.length; i++) {
            if (data[i] == '\0')
                break;
     ...
     * @return
     */
    public static int crccal(int s) {
        int str = 0;
        str = 0xAD^0x00^(s >> 8)^(s & 0xff);
        return str;
    }
